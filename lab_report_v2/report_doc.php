...
<div class="container">
	...
	<br>
    <div class="row well hidden-xs hidden-sm div_search_form"  >
...
<script>
	function changeDriver(driver_id, order_id, date,server,com,invoice,type_call){
		...
		if(type_call=='action'){
			$('#title_view_modal').html('สลับคนขับ Order '+order_id );
			if(server==1){
...
function(){
    $.post(url, {
        ...
        //------------------------------------------
    function update_lab_meet(id, status, vc,server,user,orderid) {

		if(server==1){
		   var url = "<?php echo $ip_cn;?>popup4.php?name=admin/lab_report&file=update_status&action=meet&status=" + status+"&user="+user+'&orderid='+orderid;
		}else{
		    var url = "popup4.php?name=admin/lab_report&file=update_status&action=meet&status=" + status+"&user="+user+'&orderid='+orderid;
		}
           var lat = $('#lat').val();
			var lng = $('#lng').val();
			console.log(lat+" " +lng+" : "+orderid);
            swal({
                    title: "คุณแน่ใจหรือไม่?",
